...
pipeline{
    ...
    stages {
        ...
        stage('Checkout from Git'){
            steps{
                git branch: 'dev-sec-ops-cicd-pipeline-project-one', url: 'https://github.com/kabirfaye/realworld-microservice-project.git'
            }
        }
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit kabirfaye/reddit:latest "
                       sh "docker push kabirfaye/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image kabirfaye/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.17.165.210 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.137.206.76:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#kf-devsecops-cicd-alerts', //update and provide your channel name
